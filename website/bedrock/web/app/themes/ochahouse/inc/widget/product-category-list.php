...
class JWS_PRODUCT_CATEGORY_class extends WC_Widget {
	...
	function __construct() {
        ...
        $this->widget_cssclass    = 'widget-ochahouseduct-category';
		$this->widget_description = __( 'It Displays Product Category List', 'ochahouse' );
		$this->widget_id          = 'widget-ochahouseduct-category';
		$this->widget_name        = __( 'JWS Product Category List', 'ochahouse' );
	...
	public function init_settings() {
	   ...
	   $this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Product categories', 'ochahouse' ),
				'label' => __( 'Title', 'ochahouse' ),
			),
            'hide_empty' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Hide empty categories', 'ochahouse' ),
			),
			'show_parent_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Only show parent category', 'ochahouse' ),
			),
            'hidden_category' => array(
				'type'  => 'select',
				'std'     => 'name',
				'label'   => __( 'Hidden category default','ochahouse' ),
				'options'   => $product_cat
			),
		);
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="ochahouseduct-category-list">
           <div class="container">
               <?php 
                    if(!empty($title)) {
