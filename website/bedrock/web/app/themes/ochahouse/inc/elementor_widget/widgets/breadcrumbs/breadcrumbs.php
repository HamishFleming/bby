...
class Jws_Breadcrumbs extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Jws Breadcrumbs', 'ochahouse' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'default'	=> '',
					'options'	=> [
						''		=> esc_html__( 'Current page', 'ochahouse' ),
						'id'	=> esc_html__( 'Specific page', 'ochahouse' ),
					]
				]
			);
            ...
            $this->add_control(
				'show_title',
				[
					'label' 		=> esc_html__( 'Show Title', 'ochahouse' ),
			...
			$this->add_control(
				'show_home',
				[
					'label' 		=> esc_html__( 'Show Home', 'ochahouse' ),
					'type' 			=> Controls_Manager::SWITCHER,
					'default' 		=> 'yes',
					'label_on' 		=> esc_html__( 'Yes', 'ochahouse' ),
					'label_off' 	=> esc_html__( 'No', 'ochahouse' ),
			...
			$this->add_control(
				'show_current',
				[
					'label' 		=> esc_html__( 'Show Current', 'ochahouse' ),
					'type' 			=> Controls_Manager::SWITCHER,
					'default' 		=> 'yes',
					'label_on' 		=> esc_html__( 'Yes', 'ochahouse' ),
					'label_off' 	=> esc_html__( 'No', 'ochahouse' ),
			...
			$this->add_control(
				'cpt_crumbs',
				[
					'label' 		=> esc_html__( 'CPT Crumbs', 'ochahouse' ),
					'type' 			=> Controls_Manager::SELECT,
					'default' 		=> '',
					'options'		=> [
						'' 			=> esc_html__( 'CPT Name', 'ochahouse' ),
						'terms' 	=> esc_html__( 'Taxonomy Terms', 'ochahouse' ),
						'both' 		=> esc_html__( 'Both', 'ochahouse' ),
					],
				]
			);
			...
			$this->add_control(
				'home_text',
				[
					'label' 		=> esc_html__( 'Home Text', 'ochahouse' ),
					'type' 			=> Controls_Manager::TEXT,
					'default' 		=> esc_html__( 'Homepage', 'ochahouse' ),
					'dynamic'		=> [
						'active'	=> true,
		...
		$this->start_controls_section(
			'section_separator',
			[
				'label' => esc_html__( 'Separator', 'ochahouse' ),
			]
		);
			...
			$this->add_control(
				...
				[
					...
					'default' 	=> 'icon',
					'options' 	=> [
						'text' 		=> esc_html__( 'Text', 'ochahouse' ),
						'icon' 		=> esc_html__( 'Icon', 'ochahouse' ),
					],
				]
			);
		...
		$this->start_controls_section(
			'section_item_style',
			[
				'label' 	=> esc_html__( 'Crumbs', 'ochahouse' ),
			...
			$this->add_responsive_control(
				'items_text_align',
				[
					'label' 		=> esc_html__( 'Align Text', 'ochahouse' ),
					...
					'selectors' => [
						'{{WRAPPER}} .jws-breadcrumbs,{{WRAPPER}} .elementor-widget-container h1' => 'text-align: {{VALUE}};',
					],
				]
			);
			...
			$this->start_controls_tab( 'crumb_default', [ 'label' => esc_html__( 'Default', 'ochahouse' ) ] );
			...
			$this->start_controls_tab( 'crumb_hover', [ 'label' => esc_html__( 'Hover', 'ochahouse' ) ] );
				...
				$this->add_control(
					...
					[
						...
						'default'	=> '',
						'selectors' => [
							'{{WRAPPER}} .jws-breadcrumbs__item:hover' => 'color: {{VALUE}};',
		...
		$this->start_controls_section(
			...
			[
				'label' 	=> esc_html__( 'Separators', 'ochahouse' ),
				'tab' 		=> Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' 	=> esc_html__( 'Current', 'ochahouse' ),
				'tab' 		=> Controls_Manager::TAB_STYLE,
			]
		);
	...
	protected function render() {
		...
		if ( $_query ) {
			if ( $_query->have_posts() ) {
				...
				_e( 'Post or page not found', 'ochahouse' );
		...
		if ( ! $query->is_front_page() ) { ?>
			...
			<?php
                ...
                if($settings['show_title'] == 'yes') {
                  if(is_shop()){
                           echo '<h1>'.esc_html('Shop','ochahouse').'</h1>'; 
                  }else{  
                   echo '<h1>';
			...
			if ( $query->is_archive() && ! $query->is_tax() && ! $query->is_category() && ! $query->is_tag() && ! $query->is_date() && ! $query->is_author() ) {
				...
				$this->render_item( 'tag', [
					...
					'content' 	=> sprintf( esc_html__( 'Tag: %s', 'ochahouse' ), $get_term_name ),
				] );
				...
				$this->render_item( 'year', [
					...
					'key' 		=> 'year',
					'ids' 		=> [ get_the_time('Y') ],
					'content' 	=> sprintf( esc_html__( '%s Archives', 'ochahouse' ), get_the_time('Y') ),
				...
				$this->render_item( 'month', [
					...
					'key' 		=> 'month',
					'ids' 		=> [ get_the_time('m') ],
					'content' 	=> sprintf( esc_html__( '%s Archives', 'ochahouse' ), get_the_time('F') ),
				...
				$this->render_item( 'day', [
					...
					'key' 		=> 'day',
					'ids' 		=> [ get_the_time('j') ],
					'content' 	=> sprintf( esc_html__( '%1$s %2$s Archives', 'ochahouse' ), get_the_time('F'), get_the_time('jS') ),
				] );
				...
				$this->render_item( 'author', [
					...
					'content' 	=> sprintf( esc_html__( 'Author: %s', 'ochahouse' ), $userdata->display_name ),
				] );
				...
				$this->render_item( 'search', [
					...
					'separator'	=> false,
					'key' 		=> 'search',
					'content' 	=> sprintf( esc_html__( 'Search results for: %s', 'ochahouse' ), get_search_query() ),
				] );
				...
				$this->render_item( '404', [
					...
					'separator'	=> false,
					'key' 		=> '404',
					'content' 	=> esc_html__( 'Page not found', 'ochahouse' ),
				] );
			} elseif ( is_home() ) { 
