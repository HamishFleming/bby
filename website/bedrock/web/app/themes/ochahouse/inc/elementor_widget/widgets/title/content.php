...
if ( $title ) {
	...
	$this->add_render_attribute( 'title', 'class', $class );
    	$this->add_render_attribute( 'title_shadown', 'class', 'title title_shadown' );
	$html .= sprintf( '<%1$s ' . $this->get_render_attribute_string( 'title' ) . '>%2$s</%1$s>', $tag, $title );
    if($settings['jws_enable_text_shadow']=='yes'){
    	$html .= sprintf( '<%1$s ' . $this->get_render_attribute_string( 'title_shadown' ) . '>%2$s</%1$s>', $tag, $title );
        }
