...
class Simple_Comment_Editing {
	...
	public function __construct() {
		...
		$this->errors->add( 'nonce_fail', __( 'You do not have permission to edit this comment.', 'idealauto' ) );
		$this->errors->add( 'edit_fail', __( 'You can no longer edit this comment.', 'idealauto' ) );

		$this->errors->add( 'comment_empty', __( 'Your comment cannot be empty. Delete instead?', 'idealauto' ) );
		$this->errors->add( 'comment_marked_spam', __( 'This comment was marked as spam.', 'idealauto' ) );
	...
	public function add_edit_interface( $comment_content, $passed_comment = false) {
        ...
        if(! is_singular( 'sfwd-courses' )) return $comment_content;
		...
		//Edit Button
		$sce_content .= '<div class="sce-edit-button">';
		...
		*/
		$click_to_edit_text = apply_filters( 'sce_text_edit', __( 'Click to Edit', 'idealauto' ) );
        ...
        $textarea_content .= '<fieldset class="comments-rating sce-comments-rating">';	
            $textarea_content .= '<span class="rating-container">';
	       	for ( $i = 5; $i >= 1; $i-- ) :
                ...
                $textarea_content .= '<input class="'.esc_attr($checked).'"  type="radio" id="rating-'.esc_attr( $i ).'" name="rating" value="'.esc_attr( $i ).'" /><label for="rating-'.esc_attr( $i ).'">'.esc_html( $i ).'</label>';	
			endfor;
            $textarea_content .= '<input type="radio" checked="checked" id="rating-0" class="star-cb-clear" name="rating" value="0" /><label for="rating-0">0</label>';
            $textarea_content .= '</span>';
	    $textarea_content .= '</fieldset>';
		...
		*/
		$save_text = apply_filters( 'sce_text_save', __( 'Save', 'idealauto' ) );
		...
		*/
		$cancel_text = apply_filters( 'sce_text_cancel', __( 'Cancel', 'idealauto' ) );
		...
		*/
		$delete_text = apply_filters( 'sce_text_delete', __( 'Delete', 'idealauto' ) );
	 ...
	 */
	 public function add_scripts() {
		 ...
		 */
		 ...
		 wp_enqueue_script( 'idealauto', JWS_URI_PATH. '/assets/js/education/simple-comment-editing.js' , array( 'jquery', 'wp-ajax-response', 'wp-i18n', 'wp-hooks' ), 1.0 , true );
		...
		wp_set_script_translations( 'idealauto', 'idealauto' );
		 ...
		 */
	 	...
	 	wp_localize_script( 'idealauto', 'simple_comment_editing', array(
	 		'and'                       => __( 'and', 'idealauto' ),
	 		'confirm_delete'            => apply_filters( 'sce_confirm_delete', __( 'Do you want to delete this comment?', 'idealauto' ) ),
	 		'comment_deleted'           => apply_filters( 'sce_comment_deleted', __( 'Your comment has been removed.', 'idealauto' ) ),
	 		'comment_deleted_error'     => apply_filters( 'sce_comment_deleted_error', __( 'Your comment could not be deleted', 'idealauto' ) ),
		...
		$comment_to_save = $original_comment = get_comment( $comment_id, ARRAY_A);
	 	...
	 	if ( $comment_to_save['comment_approved'] == 1 ) {
            ...
            if(function_exists('jws_removes_filter')) {
         	      jws_removes_filter( 'pre_option_comment_moderation', array( $this, 'short_circuit_comment_moderation' ) );
			     jws_removes_filter( 'pre_option_comment_whitelist', array( $this, 'short_circuit_comment_moderation' ) ); 
            }
	...
	*/
	...
	public function generate_cookie_data( $post_id = 0, $comment_id = 0, $return_action = 'ajax' ) {
		...
		$return = array(
			'name' => $cookie_name,
			'value' => $cookie_value,
			'expires' => ( current_time( 'timestamp', 1 ) ) * 1000,
			'post_id' => $post_id,
